...
- **Déploiement conditionnel.** Si le logiciel produit **doit être déployé / exploité**, ajoute une étape terminale `factory-devops` (mise en production + supervision), **dépendante de la `qa`**. Si le déploiement n'est pas dans le cadrage, écris-le comme **reste-à-faire**, jamais comme fait. N'impose pas `devops` aux livrables code/plan/librairie non déployés.
