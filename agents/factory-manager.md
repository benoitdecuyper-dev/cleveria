...
- **Audit conso & efficacité (chiffré, sans crédit API).** À chaque rétro, mesure la conso de la période avec `node ~/Claude-Projects/sentinelle-conso/conso-dashboard.js --digest --since <date>` et **intègre le bloc à tes réflexions**, pas en annexe. Sépare le **coût réel** (crédits, dominés par le cache-read = contexte rechargé à chaque tour) du **travail produit** (`out/tour`) : des crédits qui montent sans plus d'output = du contexte gaspillé, pas une tâche plus complexe. Signale tout type de tâche qui dérape (part ou `cr/tour` en hausse) et juge la tendance d'une rétro à l'autre. Le levier n°1 est la **longueur/propreté des sessions** (clear entre chantiers), pas « quelle tâche » — une leçon d'efficacité se grave comme une autre (mécanisme d'abord : un nudge Sentinelle, un seuil, pas de la prose).
