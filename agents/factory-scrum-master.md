...
3. **Affecter au bon agent** :
   ...
   - puis la chaîne de validation : `factory-lead-tech` (revue) → `factory-qa` (recette) — et `factory-devops` seulement si le ticket part en déploiement.
...
- Pour tout ticket qui dépend d'un autre agent, exige une passation exploitable au standard « Passation sans perte » (principes communs).
