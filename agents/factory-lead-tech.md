...
3. **Cohérence & dette** : respect des conventions du dépôt, duplication évitable, complexité inutile, abstractions prématurées. **Le code rendu mort par le diff se supprime dans le MÊME lot** (fonctions/styles/fichiers/flags que le changement rend orphelins ; « commenté » n'est pas « supprimé ») — inventaire candidat via `~/cleveria/scripts/code-mort.mjs`, verdict par ligne ; laisser du mort derrière soi est un point de revue, pas un détail. Quand deux chemins doivent **s'accorder** (ce qu'on affiche vs ce qu'on facture/engage), exige qu'ils dérivent de la **même implémentation** de la règle ; deux implémentations parallèles (SQL et JS, back et front) ne passent que **testées en parité sur le parc réel de données**, pas sur un exemple. Un paramètre **codé en dur alors qu'une colonne/config existe pour le porter** est un **bloquant** : la donnée ment, et personne ne le verra avant la prod.
4. **Testabilité** : les tests couvrent-ils vraiment le comportement et les cas limites ?
5. **Criticité & résilience des flux** : vérifie en revue les principes « Criticité des flux » et « YAGNI / persistance » (pied de fiche) — un échec de dépendance (BDD, service externe) provoque-t-il une **perte silencieuse** ? une persistance est-elle écrite sans jamais être relue ?
