...
- **✅ CLV-52** — État `stage` (echange|cadrage|maquette|prod) porté par l'objet + remap paresseux 1→2 + `engageProject()` + garde-fou serveur (`/api/brief` route sur `stage`, jamais sur `MODE:`). Relu lead-tech + recette QA verte, commit `c973279`. `engageProject()` pas encore câblé UI (c'est CLV-53). **Décision Benoit : promotion en place** (le même fil devient le projet, pas de fork).
- **✅ CLV-53 — FUSION COMPLÈTE** (`docs/26`). Une seule surface conversation ; `/echange` redirige vers `/voice?echange=1`. Base : `/voice` gardé, orbe lâché en V1. 6 incréments + durcissement, chacun dev → lead-tech → QA → 1 commit vert : incr.1 passerelle promotion-en-place `242111d` · durcissement verrou par id `6d3fde2` · incr.2 rail dormant `cef1f61` · incr.3 rail chaud + « Transformer en projet » + abort-mid-stream `fb81263` · incr.4 historique unifié + badge `9c16f78` · incr.5 redirect `/echange` `f401719` · incr.6 nettoyage `e254051`. Zéro régression du flux qui vend à aucun moment. **Reste (produit)** : décider d'un lien « discuter » discret sur l'accueil (l'entrée pure-chat `?echange=1` n'a pas de lien visible).
  - **⬜ Doc à réaligner** — `sourceConversationId` retiré du code mais encore décrit dans `docs/13`, `docs/22`, `docs/23` (L122/L288 disent « conservé/lu pour compat » = faux), `docs/26`. Réaligner.
  - Avancement (suite) : **✅ incr. 2** rail échange dormant (`cef1f61`) · **✅ incr. 3** rail chaud + bouton « Transformer en projet » sur /voice + séquence abort→engage→cadrage (`fb81263`, LE point dur, passé) · ⬜ incr. 4 (historique unifié) · ⬜ incr. 5 (redirect /echange→/voice) · ⬜ incr. 6 (nettoyage).
  - **⬜ CLV-55 (dette, hors CLV-53)** — Course sur l'index partagé `INDEX_KEY` entre ids **différents** (verrou par-id ne le couvre pas) → au pire perte d'un résumé de sidebar (jamais la conv). Faible proba mono-utilisateur. Fix : verrou dédié aux écritures d'index. Signalé par lead-tech + dev.
  - **⬜ CLV-56 (dette)** — `engagedAt` posé par `engageProject` mais jamais relu ET écrasé par la persistance suivante (donnée morte). À câbler (tri/export/mémoire V2) ou retirer. Signalé lead-tech incr. 3.
  - **⬜ Fiabilité tests** — flake du canari « panne de stockage » sous 6 workers Playwright (timeout, dev server webpack + SWC natif dégradé sur la machine). Passe à `--workers=2`/isolé. Fix : baisser `workers` en CI ou fiabiliser le test.
