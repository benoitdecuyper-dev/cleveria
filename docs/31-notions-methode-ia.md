...
| ✅ 5 | **Sweep adjacent + matrice de complétude** — *fait le 18/07* : `scripts/sweep-adjacent.mjs` (inventaire mécanique, verdict par ligne, `--check`, anti-écrasement) + `template-modele-fonctionnel.md` (matrice entités × cycle de vie, PO) ; câblé PO/orchestrateur/QA/Contradicteur ; recette `factory-qa` PASS 6/6 + 1 constat corrigé | 8 | Jouée : rejeu sur Sporae@`eb3fca3` (la refonte fautive) → `btn-new-contact` détecté (`prospects.html:61`), check rouge tant que non statué |
| ✅ 6 | **Les symptômes fondateurs en évals rejouables** — *fait le 18/07* : `evals/scenarios.mjs` (7 scénarios, graders code) + `scripts/run-evals.mjs` (harnais sous-agent : les hooks tirent) + `npm run evals` ; canary de règle gravé au runbook du manager | 12 | Jouée : suite **7/7 verte** — après avoir attrapé 3 vrais défauts (CDP cédant à « code-moi directement », étiquettes d'intake omises, contournement de grader), chacun devenu mécanisme |
| ✅ 7 | **Contrat de délégation + « leçon → mécanisme »** — *fait le 18/07* : frontières + échelle d'effort + lecture-parallélise/écriture-mono-exécutant dans la méthode CDP ; cliquet de **budgets de mots** (`budgets-agents.json`, `principes:check` rouge au-delà) ; leçon gravée en Partie 2 ; skill `/retro` | 10, 11, 14 | Jouée : le cliquet a attrapé **son propre auteur** le jour même (formateur 457 > 450 refusé → dégraissé) ; canary s7 rouge→vert dans la journée |
